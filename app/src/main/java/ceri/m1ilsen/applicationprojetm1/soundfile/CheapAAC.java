...
public class CheapAAC extends CheapSoundFile {
    ...
    void parseMdat(InputStream stream, int maxLen)
        ...
        for (int i = 0; i < mNumFrames; i++) {
            ...
            if(mFrameGains[i] < mMinGain)   mMinGain = mFrameGains[i];
            if(mFrameGains[i] > mMaxGain)   mMaxGain = mFrameGains[i];
            if(mProgressListener!=null && !mProgressListener.reportProgress(mOffset * 1.0 / mFileSize))
                break;
        }
    }
    ...
    void readFrameAndComputeGain(InputStream stream, int frameIndex)
        ...
        case 1:  // ID_CPE: stereo
            int windowSequence = (0x60 & data[1]) >> 5,
                    windowShape = (0x10 & data[1]) >> 4,
                    maxSfb, scaleFactorGrouping, maskPresent, startBit;
        ...
        default:
            mFrameGains[frameIndex]=(frameIndex > 0)?mFrameGains[frameIndex - 1]:0;
            break;
        }
