 ...
 * Modified by Guillaume Baudson <baudson.guillaume@hotmail.fr>
 */
public class WaveformView extends View
{
    ...
    public void zoomIn()
    {
        if(mZoomLevel >= mNumZoomLevels - 1)return;
        mZoomLevel++;
        float factor = mLenByZoomLevel[mZoomLevel] / (float) mLenByZoomLevel[mZoomLevel - 1];
        mSelectionStart *= factor;
        mSelectionEnd *= factor;
        mOffset = (int)((mOffset+(int) (getMeasuredWidth()/factor))*factor) - (int)(getMeasuredWidth()/factor);
    ...
    public double pixelsToSeconds(int pixels) {
        return (pixels * (double) mSamplesPerFrame / (mSampleRate * mZoomFactorByZoomLevel[mZoomLevel]));
    }

    public int millisecsToPixels(int msecs) {
        return (int) ((msecs * 1.0 * mSampleRate * mZoomFactorByZoomLevel[mZoomLevel]) / (1000.0 * mSamplesPerFrame) + 0.5);
    }

    public int pixelsToMillisecs(int pixels) {
        return (int) (pixels * (1000.0 * mSamplesPerFrame) / (mSampleRate * mZoomFactorByZoomLevel[mZoomLevel]) + 0.5);
    }
    ...
    public void setPlayback(int pos) {mPlaybackPos = pos;}
    ...
    protected void onDraw(Canvas canvas) {
        ...
        start = mOffset,
        width = mLenByZoomLevel[mZoomLevel] - start;

        if(width > measuredWidth)
            width = measuredWidth;

        double fractionalSecs = mOffset * pixelsToSeconds(1);

        // Draw waveform
        for(int i=0;i<width;i++)
            drawWaveform(canvas, i, start, measuredHeight, measuredHeight/2, selectWaveformPaint(i, start, fractionalSecs));
    ...
    protected void drawWaveform(final Canvas canvas, final int i, final int start, final int measuredHeight, final int ctr, final Paint paint)
    {
        ...
        drawWaveformLine(canvas, i,ctr-h, ctr+h+1, paint);
    ...
    protected Paint selectWaveformPaint(final int i, final int start, final double fractionalSecs) {
        Paint paint = (i + start >= mSelectionStart && i + start < mSelectionEnd)?
            mSelectedLinePaint:
            mUnselectedLinePaint;

        if (segmentsMap == null || segmentsMap.isEmpty())
            return paint;
        if (nextSegment == null && segmentsMap.ceilingKey(fractionalSecs) != null)
            nextSegment = segmentsMap.get(segmentsMap.ceilingKey(fractionalSecs));

        if (nextSegment == null)    return paint;
        ...
        else if(segmentsMap.ceilingKey(fractionalSecs) != null)
            nextSegment = segmentsMap.get(segmentsMap.ceilingKey(fractionalSecs));
        return paint;
    }
    ...
    protected float getGain(int i, int numFrames, int[] frameGains) {
        ...
        if (numFrames < 2)              return frameGains[x];
        else if (x == 0)                return (frameGains[0] / 2.0f) + (frameGains[1] / 2.0f);
        else if (x == numFrames - 1)    return (frameGains[numFrames - 2] / 2.0f) + (frameGains[numFrames - 1] / 2.0f);
    ...
    protected void computeDoublesForAllZoomLevels() {
        ...
        int sum = 0;
        for(minGain = 0;minGain < 255 && sum < numFrames / 20;minGain++)
            sum += gainHist[(int) minGain];

        // Re-calibrate the max to be 99%

        for(sum = 0;maxGain > 2 && sum < numFrames / 100;maxGain--)
    ...
    protected float getZoomedInHeight(float zoomLevel, int i) {
        ...
        int f = (int) zoomLevel;
        if(i % f == 0)
            return 0.5f * (getHeight(i / f - 1, mSoundFile.getNumFrames(), mSoundFile.getFrameGains(), scaleFactor, minGain, range)+
                    getHeight(i / f, mSoundFile.getNumFrames(), mSoundFile.getFrameGains(), scaleFactor, minGain, range));
        else if ((i - 1) % f == 0)
    ...
    protected float getZoomedOutHeight(float zoomLevel, int i) {
        ...
        return 0.5f * (getHeight(f, mSoundFile.getNumFrames(), mSoundFile.getFrameGains(), scaleFactor, minGain, range)+
                getHeight(f + 1, mSoundFile.getNumFrames(), mSoundFile.getFrameGains(), scaleFactor, minGain, range));
    }
    ...
    protected float getScaledHeight(float zoomLevel, int i) {
        ...
        return (zoomLevel < 1.0)?
                getZoomedOutHeight(zoomLevel, i):
                getZoomedInHeight(zoomLevel, i);
    }
}
